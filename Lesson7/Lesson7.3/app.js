class Board {
    ...
    /**
     * Является ли следующий шаг, шагом на ячейку с едой.
     * @param {Object} nextCellCoords - координаты ячейки, куда змейка собирается сделать шаг.
     * @param {number} nextCellCoords.x
     * @param {number} nextCellCoords.y
     * @returns {boolean}
     */
    isNextStepToFood(nextCellCoords) {
        let nextCell = this.getCellEl(nextCellCoords.x, nextCellCoords.y);
...
class Game {
    constructor() {
        ...
        this.bodyLengthEl = document.getElementById('body_length');
    }
     ...
     * "Пауза", "Звного", а также на стрелки на клавиатуре.
     */
    run() {
        this.menu.addButtonsClickListeners(this.start.bind(this), this.pause.bind(this), this.reset.bind(this));
    ...
    /**
     * Метод проверяет длинну змейки
     */
    CheckSnakeBodyLength() {
        this.getSnakeBodyLength();
        if (this.board.isNextStepToFood(this.snake.body[0])) {
           this.getSnakeBodyLength();
        }
    }

    /**
     * Метод выводит текст длинны змейки
     */
    getSnakeBodyLength() {
        this.bodyLengthEl.innerText = `Длина змейки: ${this.snake.body.length} / ${this.settings.winLength}`;
    }
...
class Menu {
     ...
     * событий клика на кнопки "Старт", "Пауза" и "Заного".
     * @param {Function} startBtnClickHandler 
     * @param {Function} pauseBtnClickHandler 
     * @param {Function} resetBtnClickHandler 
     */
    addButtonsClickListeners(startBtnClickHandler, pauseBtnClickHandler, resetBtnClickHandler) {
        this.startBtnEl.addEventListener('click', startBtnClickHandler);
        this.pauseBtnEl.addEventListener('click', pauseBtnClickHandler);
        this.resetBtnEl.addEventListener('click', resetBtnClickHandler);
    }
}
class Settings {
    /**
